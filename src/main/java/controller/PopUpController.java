...
public class PopUpController extends AbstractController {
    ...
    private static final String EROR_MESSAGE = "Payment denied";

    private static final String DOUBLE_PATTERN = "\\d{0,7}([\\.]\\d{0,2})?";


    @FXML private Button confirmBtn, printBtn;
    ...
    private void confirm() {
        if (receivedCashIsValid(cashInput.getText(), priceToPay)) {
            payOrders();
            showSuccessMessage();
            outlayOutput.setText(String.format("%.2f", getOutlay(priceToPay, getCashInput())));
    ...
    private void printReceipt() {
        ...
        String outlay = String.format("%.2f", getOutlay(priceToPay, getCashInput()));
    ...
    private boolean hasDoubleFormat(String value){
    ...
    private void setTableInfo(){ tableNumber.setText("TABLE " + (tableId + 1)); }
    ...
    private void showErrorMessage(){
        message.setText(EROR_MESSAGE);
        message.setTextFill(Color.web(COLOR_RED));
        message.setVisible(true);
    }

    private boolean receivedCashIsValid(String cash, Double priceToPay) {
        try {
    ...
    private Double getOutlay(Double priceToPay, Double receivedCash){
